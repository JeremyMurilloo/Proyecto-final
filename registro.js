...
form.addEventListener("submit", async function (e) {
  ...
  const { error } = await supabase
    ...
    .insert([{
      ...
      // fecha_registro y estado los maneja la BD con sus DEFAULT
    }]);

  if (error) {
    // Correo duplicado (violación UNIQUE)
    if (error.code === "23505") {
      document.getElementById("mensajeRegistro").innerHTML =
  ...
  // Éxito
  document.getElementById("mensajeRegistro").innerHTML =
    "<div class='alert alert-success'>✅ ¡Cuenta creada exitosamente! Redirigiendo al login...</div>";
